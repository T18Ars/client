...
export default function GamesTrendingPage(){
    ...
    useEffect(() => {
        const fetchDataRelate = async () : Promise<gamesSearch> => {
            const { payload } = await gamesApiRequest.getGamesTrending(page, initData.meta.page_size, sort ? sort : '0')
    ...
    const handleSort = (e: any) => {
        router.push(`/games-trending?page=${gamesSearch.meta.page}&sort=${e.target.value}`)
    }
                    ...
                    <div className="row">
                        <div className="col-lg-12">
                            <div className="breadcrumb__links">
                                ...
                                <span>{homePageT("title_trending_games")}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="row">
                        <div className="col-lg-12">
                                    ...
                                    <div className="row">
                                        <div className="col-lg-8 col-md-8 col-sm-6">
                                            <div className="section-title">
                                                <h1>{homePageT("title_trending_games")}</h1>
                                            </div>
                                        </div>
                            ...
                            <div className="product__pagination">
                                ...
                                <Link href={`/games-trending`}><i className="fa fa-angle-double-left" title="Trang đầu"></i></Link>}

                                {gamesSearch.meta.page > 1 &&
                                <Link href={`/games-trending?page=${gamesSearch.meta.page - 1}`} title="Trang trước"><i className="fa fa-angle-left"></i></Link>}
                                    ...
                                    .map((_, index) => {
                                        if ((index + 1) >= (gamesSearch.meta.page - 2) && (index + 1) <= (gamesSearch.meta.page + 2)){
                                            ...
                                            return (
                                                <Link 
                                                    key={pageNumber} 
                                                    href={`/games-trending?page=${pageNumber}`}
                                ...
                                <Link href={`/games-trending?page=${gamesSearch.meta.page + 1}`} title="Trang sau"><i className="fa fa-angle-right"></i></Link>}
                                
                                {((gamesSearch.meta.page + 1) < Number(gamesSearch.meta.total_page)) &&
                                <Link href={`/games-trending?page=${gamesSearch.meta.total_page}`} title="Trang cuối"><i className="fa fa-angle-double-right"></i></Link>}
                            </div>
                        </div>
                    </div>
                </div>
