    ...
    useEffect(() => {
        if(game && game.length > 2){
            const fetchDataRelate = async () : Promise<gamesSearch> => {
                const { payload } = await gamesApiRequest.getGamesSearchPagi(game, page, initData.meta.page_size, sort ? sort : '0')
            ...
            setGamesSearch(initData)
        }
    }, [page, sort, game])

    const handleSort = (e: any) => {
        router.push(`/search?game=${game}&page=${gamesSearch.meta.page}&sort=${e.target.value}`)
    }
                    ...
                    <div className="row">
                        <div className="col-lg-12">
                            ...
                            <div className="product__pagination">
                                ...
                                <Link href={`/search?game=${game}`}><i className="fa fa-angle-double-left" title="Trang đầu"></i></Link>}

                                {gamesSearch.meta.page > 1 &&
                                <Link href={`/search?game=${game}&page=${gamesSearch.meta.page - 1}`} title="Trang trước"><i className="fa fa-angle-left"></i></Link>}
                                    ...
                                    .map((_, index) => {
                                        if ((index + 1) >= (gamesSearch.meta.page - 2) && (index + 1) <= (gamesSearch.meta.page + 2)){
                                            ...
                                            return (
                                                <Link 
                                                    key={pageNumber} 
                                                    href={`/search?game=${game}&page=${pageNumber}`}
                                ...
                                <Link href={`/search?game=${game}&page=${gamesSearch.meta.page + 1}`} title="Trang sau"><i className="fa fa-angle-right"></i></Link>}
                                
                                {((gamesSearch.meta.page + 1) < Number(gamesSearch.meta.total_page)) &&
                                <Link href={`/search?game=${game}&page=${gamesSearch.meta.total_page}`} title="Trang cuối"><i className="fa fa-angle-double-right"></i></Link>}
                            </div>
                        </div>
                    </div>
                </div>
